...
def get_features():
    ...
    cifar100_dataloader, test_loader, augmented_train_loader, cifar100_dataset = get_data_loaders(dataset)

    # 提取特征向量
    features = []
    with torch.no_grad():
        for images, _ in cifar100_dataloader:
            outputs = model(images)
            features.extend(outputs.squeeze().cpu().numpy())

    features = np.array(features)
    true_labels = cifar100_dataset.targets
...
def get_all_labels():
    ...
    np.savetxt('./data/cluster_labels_ cifar100.txt', hc_labels, fmt='%d')
