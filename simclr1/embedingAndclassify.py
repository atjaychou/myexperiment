...
# def saveModelCheckpoints(model, epoch, optimizer, loss, best_acc):
#     checkpoint = {
#         'epoch': epoch,
...
def train(model, train_loader, augmented_train_loader, test_loader, cluster_labels, learning_rate, temperature,
    ...
    for epoch in range(num_epoch):
        ...
        for batch_index, (train_data, augmented_train_data) in enumerate(zip(train_loader, augmented_train_loader)):
            ...
            embX, train_features, logits_x = model(input1)
            embY, augmented_features, logits_y = model(input2)
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), './checkpoints/model/best_model.pth')

        saveModelCheckpoints(model, epoch, optimizer, check_loss, best_acc)
