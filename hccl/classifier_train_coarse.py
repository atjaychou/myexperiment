...
def train_classifier(model, train_loader, test_loader, learning_rate):
    ...
    logging.basicConfig(filename='/data/Tian/zxj/cc/hccl/result/classification_coarse_'+time_string+'.log', level=logging.INFO)
...
def get_test_loader():
    ...
    train_dataset.targets = sparse2coarse(train_dataset.targets)  # update labels
    test_dataset = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=True, transform=test_transform)
    test_dataset.targets = sparse2coarse(test_dataset.targets)  # update labels
    # DataLoader
...
def main():
    ...
    n_classes = config["n_classes_coarse"]
