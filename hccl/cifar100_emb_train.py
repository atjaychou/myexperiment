...
from hccl.classifier_train_fine import test_model_fine
...
def data_augmentation(image_size=96):
...
def get_data_loaders(dataset_name):
    ...
    if dataset_name == 'cifar10':
        # CIFAR-10 dataset
        train_dataset1 = datasets.CIFAR10(root=data_dir, train=True, download=True, transform=data_augmentation)
        ...
        train_dataset2 = torchvision.datasets.CIFAR10(root=data_dir, train=True, download=True,
                                                          transform=data_augmentation)
        cifar10_datasets = [train_dataset1, train_dataset2]

        torch.manual_seed(seed=seed)
        g = torch.Generator()
        train_loader = DataLoader(cifar10_datasets, batch_size=batch_size, shuffle=True,
        ...
        # CIFAR-100 dataset
        train_dataset1 = datasets.CIFAR100(root=data_dir, train=True, download=True, transform=data_augmentation)
        ...
        train_dataset2 = torchvision.datasets.CIFAR100(root=data_dir, train=True, download=True,
                                                           transform=data_augmentation)
        cifar100_datasets = [train_dataset1, train_dataset2]
        torch.manual_seed(seed=seed)
        train_loader = DataLoader(cifar100_datasets, batch_size=batch_size, shuffle=False,
                                  num_workers=0, drop_last=True)
        print("File ready")
...
def saveModelCheckpoints(model, epoch, time_string):
    checkpoint = {
        'epoch': epoch,
...
def train(model, train_loader, augmented_train_loader, cluster_list, learning_rate, temperature,
    ...
    for epoch in range(num_epoch):
        ...
        for batch_index, (train_data, augmented_train_data) in loop:
            ...
            loss = (my_loss(train_layer1_projection, augmented_layer1_projection, cluster_list[0], cluster_list[3], batch_index, temperature, param1)).to(device)
        ...
        PATH = saveModelCheckpoints(model, epoch, time_string)
        test_model_fine(PATH)

    print('finish training')


def get_cluster_list(train_cluster_path, aug_cluster_path):
    cluster_train_labels = np.loadtxt(train_cluster_path, dtype=int)
    cluster_aug_labels = np.loadtxt(aug_cluster_path, dtype=int)
    ...
    cluster_train_labels3 = [item[2] for item in cluster_train_labels]
    cluster_train_labels3 = np.array(cluster_train_labels3)
    ...
    cluster_aug_labels3 = [item[2] for item in cluster_aug_labels]
    cluster_aug_labels3 = np.array(cluster_aug_labels3)

    cluster_list = [cluster_train_labels1, cluster_train_labels2, cluster_train_labels3, cluster_aug_labels1,
                    cluster_aug_labels2, cluster_aug_labels3]
