...
def train(model, train_loader, augmented_train_loader, learning_rate,
          temperature=0.5,
          margin=1.0, batch_size=64, num_epochs=10):
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)
    # 提取训练集的特征向量
    train_features, train_labels = extract_features(train_loader, model)
    augmented_features, augmented_labels = extract_features(augmented_train_loader, model)
    model.train()
    ...
    for epoch in range(num_epochs):
        ...
        train_features = torch.tensor(train_features, requires_grad=True)

        loss = my_loss(train_features, augmented_features, cluster_labels, temperature).to(device)
...
def main():
    ...
    num_epochs = 10
    learning_rate = 0.01
    train(model, train_loader, augmented_train_loader, learning_rate)
